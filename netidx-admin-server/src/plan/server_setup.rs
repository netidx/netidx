...
pub struct SetupArgs<'a> {
    ...
    /// The trust domain's TLS domain — what discovery groups by.
    pub domain: &'a str,
...
/// admin server is a trust domain daemon, so it needs a **system**-scope
...
pub async fn setup_server(
    ...
    let kc = transport::generate_key_and_csr(SERVING_SAN)?;
    ...
    admin_server_config::save_async(&a.config_lock, &cfg_path, &cfg).await?;
    ...
    // An admin server is a trust domain daemon → system-scope service.
...
pub async fn update_roles(
    ...
    let mut cfg = admin_server_config::load_async(&cfg_path).await?;
    update(&mut cfg.roles);
    admin_server_config::save_async(config_lock, &cfg_path, &cfg).await?;
    Ok(cfg_path)
}
...
pub async fn set_ca_autorenew_at(
    ...
    let mut cfg = admin_server_config::load_for_recovery_async(&cfg_path).await?;
    ...
    admin_server_config::validate_async(&cfg)
        .await
