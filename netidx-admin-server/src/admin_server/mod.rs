...
struct Server {
    ...
    roots: RootCertStore,
    pki_client: transport::PkiClient,
...
impl Server {
    ...
    fn from_state(
        ...
        let pki_client = transport::PkiClient::new(roots.clone())?;
        let outbound_tls = if serving_cert_pem.is_empty() || serving_key_pem.is_empty() {
            ...
            Some(CachedOutboundClient {
                ...
                client: transport::AuthenticatedPkiClient::from_pem(
                    roots.clone(),
                    &serving_cert_pem,
                    &serving_key_pem,
                )?,
    ...
    async fn outbound_client(&self) -> Result<transport::AuthenticatedPkiClient> {
        ...
        let client =
            transport::AuthenticatedPkiClient::from_pem(self.roots.clone(), &cert, &key)
