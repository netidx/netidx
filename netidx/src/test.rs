...
mod publisher {
    ...
    // Regression test for the id-map resilience fix: once a TLS identity has
    // been mapped, a subsequent id-map outage must NOT deny it — the resolver
    // serves the last-known-good mapping (mirroring the TLS reload fallback)
    // instead of failing the resolve. id_map_timeout=0 forces the mapper to be
    // consulted on every resolve, so without the fallback every cold subscribe
    // after the outage would fail. A flaky Command mapper backed by a sentinel
    // file stands in for a dead/unreachable id-map daemon.
    #[cfg(unix)]
    #[tokio::test(flavor = "multi_thread")]
    async fn idmap_outage_serves_last_known() {
        use crate::resolver_server::config::IdMap;
        use std::os::unix::fs::PermissionsExt;
        let _ = env_logger::try_init();
        let dir =
            std::env::temp_dir().join(format!("netidx-idmap-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let script = dir.join("idmap.sh");
        let sentinel = dir.join("down");
        // Up while the sentinel is absent; "down" (exit 1) once it appears.
        std::fs::write(
            &script,
            format!(
                "#!/bin/bash\nif [ -f {down} ]; then exit 1; fi\n\
                 echo \"uid=1000(user) gid=1000(user) groups=1000(user),4(adm)\"\n",
                down = sentinel.display()
            ),
        )
        .unwrap();
        std::fs::set_permissions(&script, std::fs::Permissions::from_mode(0o755)).unwrap();
        let mut server_cfg =
            ServerConfig::load("../cfg/tls/resolver/resolver.json").unwrap();
        server_cfg.member_servers[0].id_map =
            IdMap::Command(arcstr::ArcStr::from(script.to_str().unwrap()));
        server_cfg.member_servers[0].id_map_timeout = chrono::Duration::zero();
        let server = Server::new(server_cfg, false, 0).await.expect("start server");
        let mut cfg = ClientConfig::load("../cfg/tls/client/client.json").unwrap();
        cfg.addrs[0].0 = *server.local_addr();
        ...
        task::spawn(run_publisher(cfg.clone(), default_destroyed, tx, auth.clone()));
        time::timeout(Duration::from_secs(5), ready).await.unwrap().unwrap();
        // Warm the cache for the client identity while the mapper is up.
        Subscriber::new(cfg.clone(), auth.clone())
            .unwrap()
            .subscribe_nondurable_one("/app/v0".into(), Some(Duration::from_secs(5)))
            .await
            .expect("warm-up subscribe should succeed while id-map is up");
        // The id-map daemon goes down.
        ...
        drop(server);
        let _ = std::fs::remove_dir_all(&dir);
        assert_eq!(fail, 0, "id-map outage denied a known identity {fail}/15 times");
    }
