...
pub(crate) mod resolver {
    ...
    fn publisher_id() -> impl Strategy<Value = PublisherId> {
        any::<u64>().prop_map(|i| unsafe { PublisherId::mk(i) })
    }
    ...
    pub(crate) fn from_read() -> impl Strategy<Value = FromRead> {
        prop_oneof![
    ...
    pub(crate) fn to_write() -> impl Strategy<Value = ToWrite> {
        prop_oneof![
    ...
    pub(crate) fn from_write() -> impl Strategy<Value = FromWrite> {
        prop_oneof![
...
pub(crate) mod publisher {
    ...
    pub(crate) fn to() -> impl Strategy<Value = To> {
        prop_oneof![
            ...
            any::<u64>().prop_map(|i| To::Unsubscribe(unsafe { Id::mk(i) })),
    ...
    pub(crate) fn from() -> impl Strategy<Value = From> {
        prop_oneof![
            path().prop_map(From::NoSuchValue),
            path().prop_map(From::Denied),
            any::<u64>().prop_map(|i| From::Unsubscribed(unsafe { Id::mk(i) })),
            ...
            (any::<u64>(), value())
                .prop_map(|(i, v)| From::Update(unsafe { Id::mk(i) }, v)),
            Just(From::Heartbeat),
            (any::<u64>(), value())
                .prop_map(|(i, v)| From::WriteResult(unsafe { Id::mk(i) }, v))
        ]
    }
