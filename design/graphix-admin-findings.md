...
| 2026-09-21 | 3883 | END TO END, the real binary (`netidx admin tui` under a pty, exec → first byte on the terminal): RELEASE warm 65–69ms pinned to a P core / 76–91ms unpinned, cold 826–836ms; `quick` profile (thin LTO, 16 cgu) warm 70–73 / 83–88ms, cold 953ms. In-process `milestone_image_fused` on `quick`: warm 56ms, cold 884ms (1335 kernels attempted, 380 fused), image 6.35MB | The first reading through the driver. The in-process warm number is where it was (54–55ms in graphix `design/program_image.md`, slice (d)), so the image is healthy; a real launch adds ~4ms of process start and ~10ms of terminal init, first draw and the cycles up to the frame. It also added 12.5ms that was pure waste: the image cache key read the WHOLE executable (109MB on `quick`) for the build-id note in its first page; mapped now (graphix `8e2a1e55`), which is the 82 → 71ms the pinned `quick` reading moved. Unpinned costs ~15ms because the runtime thread lands on an E core. `quick` is within ~8% of release warm and ~13% cold: good enough to measure with. Cold is dominated by the compile, and the first frame also waits for the image encode + write (`Shell::init` awaits both images before the display starts). Exit is 30–48ms of single-threaded drop of the graph. Harness: a pty + timestamps, in the session scratchpad; not checked in. |
| 2026-09-22 | 3883 | END TO END, release, pinned: cold 830 → 593–600ms, warm 65–69ms (unchanged). `quick` in-process `milestone_image_fused`: cold 865 → ~630ms | The cold write, not the compile, was the slack: the program image write was 349ms of which the actual encode was 87ms. Three graphix fixes: (1) every heap-deferred instance's reference summary was re-walked in each pass, and an enclosing instance's walk descended into nested ones — cached per instance now; (2) ids were renumbered densely then sorted, which forced a SECOND full measure pass to get widths right (40% of the write) — ids are written as minted and the header carries each domain's span (`b7a7cf65`, format 10). The fuzzer found the naive version: reserving the extent rather than the span compounds across sessions until the counter overflows; (3) an embedded (`Source::Internal`) program no longer writes the registration image, which only a different program would read (40ms). Write is now measure 98 + encode 53 = 151ms. Not done: the measure pass is still 2x the encode; a single-pass writer that measures on demand would take another ~50ms but touches the slot state machine. |
...
16. **A seq let's fire is only live in the next step** (09-04). Later
    ...
    immediately after the let. `do { }` is now a seq statement: one arm,
    lets inside, connects pc-sampled. Skip with `_ => null`.
