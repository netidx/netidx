...
- **Hardware token / PKCS#11 backing.** v1 supports optional PKCS#8
  ...
  entirely.
- **Remote-admin domain CA operations.** v1 CA is local-machine only.
...
- **Bootstrap trust on first connect.** Admin-server's own cert is
  ...
  admin domain the discovery channel can be spoofed; `--admin-server-pin
...
- **Cross-subnet discovery.** Multicast only crosses subnets with an
  explicit relay. Multi-subnet admin domains use explicit `--admin-server
  <addr>`.
