...
case of "ask the admin domain instead of the human." The admin server is the
...
**one** fingerprint glyph per admin domain, ever, instead of one per device
pair.

## Trust model

1. **The admin plane is always TLS, rooted at the CA — regardless of
   data-plane auth.** On a Kerberos (or anonymous) admin domain the CA's
...
2. **mDNS beacons are hints, never trusted.** TXT carries the domain,
   ...
   nothing.
3. **One glyph per admin domain.** The CA fingerprint confirm (text +
...
4. **Reserved serving SAN.** Every admin server serves TLS with a
   ...
   the local setup path on the CA host or by the policy-gated admin domain
   **Enroll**.
...
- **GetInfo** returns *local facts + known peers*: the admin domain's domain name,
  ...
  One reachable admin server maps the whole admin domain, so the manual
...
verifies the *admin domain* by the CA fingerprint, and the admin verifies
...
- **CA rollover rides the same rails**: accepting the *returned* trust
  ...
  leaves still chain and the admin domain glyph (a key hash) is unchanged.
...
- **Workstation / publisher**: browse → pick the domain (asked only if
  ...
  fetch + glyph-confirm the admin domain identity → aggregate GetInfo across
  its admin servers → the parent referral gets **every** resolver with
  its per-address auth. Then:
  - TLS admin domain: one join (suggested SAN `user.<domain>`, id-map
    groups for the new identity — default `users` — then admin +
    password); registration arrives via the CA push.
  - Krb5 admin domain: configs are written with per-address `Krb5 { spn }`;
...
- **First resolver**: per-box single-member resolver config (the
  ...
  resolvers stay mutually unaware). The CA is created for TLS admin domains
  without asking — it signs the data plane anyway — and for krb5
  admin domains too, scoped to the admin plane. `setup_server` writes a
