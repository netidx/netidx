...
enum ToRt {
    ...
    Subscribe { ch: mpsc::Sender<RtEvent>, res: oneshot::Sender<()> },
}
...
impl BSHandle {
    ...
    pub async fn subscribe(&self, ch: mpsc::Sender<RtEvent>) -> Result<()> {
        Ok(self.exec(|tx| ToRt::Subscribe { ch, res: tx }).await?)
    }
}
