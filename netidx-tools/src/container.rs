...
pub(crate) async fn run(cfg: Config, auth: DesiredAuth, params: Params) -> Result<()> {
    ...
    // Wait for a shutdown request (ctrl-c, SIGTERM, or the activation
    // supervisor's Windows shutdown event), then drop the container —
    // flushing sled cleanly rather than relying on crash recovery.
