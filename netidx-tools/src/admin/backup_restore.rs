...
use clap::Args;
#[cfg(unix)]
use netidx_admin::plan::{AuthKind, install::resolver::enroll_admin_server};
use netidx_admin::{
    admin_client,
...
pub(crate) struct RestoreArgs {
    ...
    /// Read the controller recovery password from stdin.
    ...
    /// bundled controller certificate has expired.
    ...
    /// Protection for freshly enrolled non-controller TLS keys.
    ...
    /// Override the controller/bootstrap address recorded in the bundle.
    ...
    #[arg(long = "for-user")]
    pub for_user: Option<String>,
    /// Permit plaintext replacement controller credentials when no TPM or
...
pub(crate) fn backup(a: BackupArgs) -> Result<()> {
    ...
    let has_controller = root.join("ca").is_dir();
    #[cfg(unix)]
    let controller_tmp =
        tempfile::tempdir().context("creating controller-backup staging directory")?;
    let controller: Option<PathBuf> = if has_controller {
        #[cfg(unix)]
        {
            let cfg = root.join("admin-server.json");
            let inner = controller_tmp.path().join("controller");
            tokio::runtime::Runtime::new()?
                .block_on(netidx_admin::admin_local::backup(&cfg, &inner))?;
            Some(inner)
        }
        #[cfg(not(unix))]
        {
            bail!("a controller backup can only be captured on its unix host")
        }
    } else {
        None
    };
...
pub(super) async fn network_for_restore(
    ...
    let first = {
        ...
        let mut found = None;
        for _ in 0..30 {
            match admin_client::fetch_identity(seed, NodeKind::Client).await {
                Ok(identity) => {
                    found = Some(identity);
                    break;
                }
    ...
    if !network.matches(&first.fingerprint)? {
        bail!("the server at {seed} belongs to a different CA than the backup");
    }
    let info = admin_client::aggregate(&[seed], NodeKind::Client, &first).await?;
    let controller =
        info.ca_addr.context("the verified network reported no controller")?;
    let identity = admin_client::fetch_identity(controller, NodeKind::Client).await?;
    if !identity.controller || !network.matches(&identity.fingerprint)? {
        bail!("the discovered controller did not match the backup's pinned CA identity");
    }
    let info =
        admin_client::aggregate(&[controller], NodeKind::Client, &identity).await?;
...
pub(super) fn identities_complete(
    ...
    manifest.identities.iter().all(|recipe| {
        if recipe.kind == IdentityKind::AdminServer {
            #[cfg(unix)]
            return netidx_admin::admin_server_config::AdminServerConfig::load(
...
pub(super) async fn reenroll_satellite_admin(
    ...
    if let Ok(cfg) =
        netidx_admin::admin_server_config::AdminServerConfig::load(&config_path)
        && Some(cfg.server_id) != manifest.previous_admin_server
        && install_bundle::identity_files_usable(
            &cfg.serving_cert,
            &cfg.serving_key,
...
/// The recovered controller must start before its co-located identities can
...
/// the controller) are never restarted here.
...
pub(crate) fn restore(a: RestoreArgs) -> Result<()> {
    ...
    let has_controller = preflight.components.contains(&Component::Controller);
    if has_controller && !a.old_controller_fenced {
        bail!(
            "controller restore requires --old-controller-fenced; do not continue until the old controller cannot run"
        );
    }
    ...
    if has_controller {
        ...
        if !install_bundle::controller_recovered(&bundle, &cfg)? {
            if !install_bundle::controller_snapshot_prepared(&bundle, &ca_dir, &cfg)? {
                ...
                .context("restoring the verified controller state")?;
            }
    ...
    if !manifest.identities.is_empty() && !identities_complete(&root, &manifest) {
        let mut ans = answer_cli::FlagAnswerer::install(
            ...
            manifest
                ...
                .as_ref()
                .map(|n| {
                    netidx_admin::fingerprint::Fingerprint::parse_text(&n.ca_fingerprint)
                })
                .transpose()?,
        )?;
