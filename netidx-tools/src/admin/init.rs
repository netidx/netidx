...
pub(crate) struct WorkstationFlags {
    ...
    /// admin domain subscribers must reach.
    #[arg(long = "default-auth")]
    default_auth: Option<AuthKind>,
    /// Base path of the local resolver admin domain (default `/local`).
...
pub(crate) struct WorkstationJoinFlags {
    ...
    /// `password`, or `none`. Only relevant when joining a TLS admin domain
    /// (where `join` enrolls a client certificate).
    #[arg(long = "key-protection")]
    pub key_protection: Option<KeyProtArg>,
    /// The admin domain's admin server (`ip:port`, or a host resolved with the
    /// default admin port). Names the admin domain directly instead of discovering
    /// it — required in strict mode (discovery is interactive-only).
    #[arg(long = "admin-server")]
    pub admin_server: Option<String>,
    /// The admin domain's CA fingerprint, obtained out of band; confirms the
    /// admin domain's identity non-interactively (required with `--admin-server`).
    #[arg(long = "accept-glyph")]
...
pub(crate) fn run_workstation_join(f: WorkstationJoinFlags) -> Result<()> {
    ...
    // password); the admin domain is named by --admin-server + glyph-confirmed by
...
pub(super) fn show_admin_domain_identity(
    ...
    println!(
        "The admin server at {addr} serves admin domain {:?} (roles: {}) and presented \
         this identity:",
        identity.domain,
...
pub(crate) struct ResolverFlags {
    ...
    /// private-admin domain IPv4, else loopback. IPv6 is accepted if you
    ...
    /// Explicitly set up an admin server for this admin domain. Only meaningful on an
    ...
    /// Set this resolver up as a CHILD of an existing admin domain: give the
...
pub(crate) struct PublisherFlags {
    /// Admin domain address (repeatable). All addresses share the auth
