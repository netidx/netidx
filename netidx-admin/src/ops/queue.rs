...
use crate::{
    ...
    transport::{self, csr_fingerprint},
};
use anyhow::Result;
...
async fn fetch_queue(sess: &super::AdminSession) -> Result<Vec<QueueItem>> {
    Ok(transport::list_queue(sess.server, sess.credential.clone(), &sess.identity)
        .await?
        .into_iter()
        .map(to_item)
        .collect())
...
pub async fn approve(
    ...
    let warnings = transport::approve(
        sess.server,
