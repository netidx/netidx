...
async fn recover_controller_with_password_and_lock(
    ...
    let mut cfg = AdminServerConfig::load_for_recovery_async(config_path).await?;
    ...
    let mut map = netmap::load_async(ca_dir, cfg.server_id).await?;
...
mod tests {
    ...
    async fn restored_controller_rebinds_to_new_machine_without_changing_identity() {
        ...
        let map = netmap::load(fixture.ca_dir.as_path(), fixture.server_id).unwrap();
        assert_eq!(
    ...
    async fn running_controller_backs_up_over_the_protected_local_rpc() {
        ...
        let status = crate::admin_local::ca_status(&config).await.unwrap();
    ...
    async fn controller_recovery_rejects_wrong_password_and_identity_mismatch_before_writes()
     {
        ...
        let before_map = std::fs::read(netmap::path(fixture.ca_dir.as_path())).unwrap();
        assert!(
            recover_controller_with_password(
                fixture.ca_dir.as_path(),
                &fixture.config,
        ...
        assert_eq!(
            std::fs::read(netmap::path(fixture.ca_dir.as_path())).unwrap(),
            before_map
        );
