...
fn selected_server_sets(
    map: &crate::admin_proto::NetworkMap,
...
pub async fn delegate_under_parent(
    ...
    let map = admin_client::get_map_pinned(parent_conf_addr, NodeKind::Client, &identity)
        .await?;
    let parent_id = map
        .servers
        .iter()
    ...
    .await?;
    let code =
        admin_client::delegation_code(proposed_path, &servers.parent, &servers.child);
