...
use crate::admin_proto::DEFAULT_PORT;
...
/// and on non-TLS networks the admin-plane CA that anchors it).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum AdminPlane {
...
/// scheme.
///
/// TLS already creates the CA on a fresh network (it signs the data plane),
/// and krb5/anonymous still need the admin plane's TLS trust root for
/// discovery, enrollment, and renewal — declining it on a TLS or krb5 network
/// produces a network where certificate renewal and zero-touch installs can
/// never work, so neither is offered as a question. Anonymous networks may
...
mod tests {
    ...
    fn the_admin_plane_matrix() {
        use AdminPlane::*;
        use AuthKind::*;
        // Declining the admin plane on a TLS or krb5 network breaks renewal +
        // zero-touch installs forever, so neither is a question — fresh
        // network or joining one (enrollment queues for remote approval, so no
        // admin is needed at this keyboard). Anonymous networks may not want
