...
pub fn identity_recipes(root: &Path, role: InstallRole) -> Result<Vec<IdentityRecipe>> {
    ...
    #[cfg(unix)]
    {
        ...
        if config.is_file() {
            let cfg = crate::admin_server_config::AdminServerConfig::load(&config)?;
            push_identity(
                &mut out,
                root,
...
pub fn create(
    ...
    #[cfg(unix)]
    let admin_config = crate::admin_server_config::AdminServerConfig::load(
...
pub fn controller_recovered(bundle: &Path, config: &Path) -> Result<bool> {
    ...
    let cfg = match crate::admin_server_config::AdminServerConfig::load(config) {
        Ok(cfg) => cfg,
...
pub fn controller_snapshot_prepared(
    ...
    let cfg: crate::admin_server_config::AdminServerConfig =
        match crate::admin_server_config::AdminServerConfig::load_for_recovery(config) {
            Ok(cfg) => cfg,
            Err(_) => return Ok(false),
        };
    ...
    if crate::fingerprint::Fingerprint::of_cert_pem(&live_cert)?.text()
...
pub fn restore_files(bundle: &Path, root: &Path) -> Result<Manifest> {
    ...
    if root.exists() {
        ...
        if manifest
            ...
            && crate::admin_server_config::AdminServerConfig::load(
                &root.join("admin-server.json"),
            )
            .is_ok_and(|cfg| {
