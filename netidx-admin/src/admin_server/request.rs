...
use super::{
    Server,
    admins::{
        handle_add_role_admin_prepared, handle_change_password, handle_list_admins,
        handle_login, handle_logout, handle_remove_admin, handle_reset_password,
...
/// The same question for the ops that derive a *new* slot's Argon2 before
/// taking the write lock (add-role, change-password, reset-password): the
/// reason not to prepare, or `None` to go ahead.
///
/// `local` is exempt for the reason above — the empty credential a local
/// request sends is not a failed login, it is the absence of one, because
/// `SO_PEERCRED` already authorized it. Without the exemption `ca admin
/// add-role` on the CA host answered "authentication failed" no matter what,
/// since an empty credential can never authenticate.
fn prepare_blocked_by_credential(
    local: bool,
    authentication: &PreparedAdminAuthentication,
) -> Option<String> {
    if local { None } else { authentication.credential_failure() }
}
    ...
    S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin + Send,
{
    ...
    let result = match req {
        ...
        Request::ChangePassword(req) => {
            ...
            let prepared = match prepare_blocked_by_credential(local, authentication) {
                Some(reason) => Err(reason),
                // The local control socket has no keyslot behind it, so this
                // always refuses — don't spend an Argon2 to find that out.
                _ if local => Err("no keyslot".to_string()),
                // Checked here, before the Argon2: the client checks too, but
                // that is for the error message. Anything reachable over the
                // wire is reachable by something that isn't our client, and an
                // empty password would make the admin name the whole
                // credential.
                None => match crate::ops::roster::check_new_password(
                    req.new_password.as_str(),
                ) {
                    Err(e) => Err(format!("{e:#}")),
                    Ok(()) => match authentication.admin_name() {
                        Some(admin) => {
                            prepare_role_rekey(
                                state,
                                &signs,
        ...
        Request::ResetPassword(req) => {
            let authentication = request_authentication();
            let prepared = match prepare_blocked_by_credential(local, authentication) {
                Some(reason) => Err(reason),
                // Same reason as change-password: the key is generated by the
                // caller, and an empty one would make the target's account
                // free to anyone who can name it.
                None => match crate::ops::roster::check_new_password(
                    req.new_password.as_str(),
                ) {
                    Err(e) => Err(format!("{e:#}")),
            ...
            let resp =
                handle_reset_password(state, &req, authentication, local, prepared).await;
            admin_proto::write_msg(&mut tls, &resp)
                .await
                .context("writing AdminMgmtResponse")
        }
