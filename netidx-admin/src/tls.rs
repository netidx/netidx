...
pub use netidx_admin_proto::identity::{
    AdminCertIdentity, admin_cert_identity_from_der, admin_cert_identity_from_pem,
};
...
mod tests {
    ...
    fn encrypt_decrypt_round_trips_the_key() {
        let kc = crate::transport::generate_key_and_csr("x.example.com").unwrap();
        let enc =
    ...
    fn admin_cert(uris: &[String]) -> Vec<u8> {
        ...
        let mut params =
            CertificateParams::new(vec![netidx_admin_proto::SERVING_SAN.to_string()])
                .unwrap();
    ...
    fn protocol_v6_admin_certificate_identity_is_strict() {
        ...
        let malformed = admin_cert(&[format!(
            "{}not-a-uuid",
            netidx_admin_proto::SERVER_ID_URI_PREFIX
        )]);
